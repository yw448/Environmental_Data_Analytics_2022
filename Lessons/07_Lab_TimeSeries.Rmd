...
ex6.rain=ggplot(BRA_complete,aes(x=Date,y=Rainfall))+
  #geom_point()+
  geom_line()+
  geom_smooth(method = "lm")
print(ex6.rain)

ex6.temp=ggplot(BRA_complete,aes(x=Date,y=Temperature_C))+
  #geom_point()+
  geom_line()+
  geom_smooth(method = "lm")
print(ex6.temp)
...
ex7.rain= ts(BRA_complete$Rainfall,start = c(f_year,f_month),frequency = 12)
ex7.temp= ts(BRA_complete$Temperature_C,start = c(f_year,f_month),frequency = 12)
...
ex8.rain=stl(ex7.rain,s.window = "periodic")
plot(ex8.rain)
ex8.temp=stl(ex7.temp,s.window = "periodic")
plot(ex8.temp)
...
ex9.rain=as.data.frame(ex8.rain$time.series[,1:3])
ex9.rain=mutate(ex9.rain, Observed=BRA_complete$Rainfall, Date=BRA_complete$Date)
ex9.temp=as.data.frame(ex8.temp$time.series[,1:3])
ex9.temp=mutate(ex9.temp, Observed=BRA_complete$Temperature_C, Date=BRA_complete$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series

ex10.rain=ggplot(ex9.rain)+
  geom_line(aes(x=Date,y=Observed),size=0.25)+
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff")#+
  #geom_hline(yintercept = 0, lty = 2)
print(ex10.rain)
ex10.temp=ggplot(ex9.temp)+
  geom_line(aes(x=Date,y=Observed),size=0.25)+
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff")#+
  #geom_hline(yintercept = 0, lty = 2)
print(ex10.temp)
