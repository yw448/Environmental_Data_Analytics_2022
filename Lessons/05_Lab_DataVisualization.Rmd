...
EX1 <- ggplot(subset(PeterPaul.chem.nutrients,depth == 0 ), aes(x=daynum, y=temperature_C, color = year4))+geom_point()+facet_wrap(vars(lakename), nrow=2)+ylab("Surface Temperature")
...
EX2 = ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth))+geom_point(size=0.5)+scale_color_viridis(option = "magma", direction = -1)+
...
EX3 <- ggplot(EPAair, aes(x=PM2.5,y=Ozone, color=meanLat))+
  geom_point(alpha=0.5)+geom_smooth(method = lm,color="red")+
...
ggplot(EPAair, aes(x=Month,y=PM2.5))+geom_point()+
  facet_wrap(vars(Year), nrow = 2)+scale_x_continuous(limits = c(1,12),breaks = c(1,12))
