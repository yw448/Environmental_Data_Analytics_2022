...
* ***Easy to adjust***
* ***Can create various kind of outputs***
...
data=Lake[1:6,]
knitr::kable(data,caption="Lake nutrients")
```
...
```{r Installing packages, eval=FALSE, message=FALSE, include=FALSE}
...
```{r echo=FALSE}
summary1=Lake.processed %>%
  group_by(lakename) %>%
  summarize(mean=mean(tn_ug),min=min(tn_ug),
            max=max(tn_ug),STD=sd(tn_ug))
summary2=Lake.processed %>%
  group_by(lakename) %>%
  summarize(mean=mean(tp_ug),min=min(tp_ug),
            max=max(tp_ug),STD=sd(tp_ug))
```
...
```{r echo=FALSE}
kable(summary1,caption = "Nitrogen")
kable(summary2, caption = "Phosphorus")
```
...
```{r echo=FALSE}
plotN= ggplot(Lake.processed,
              aes(x=sampledate,y=tn_ug,color=lakename))+
  geom_line()+
  labs("Nitrogen")
print(plotN)
plotP= ggplot(Lake.processed,
...
Nutrient concentrations in different lakes have seasonal changes with similar trend.

***

I might take a time series analysis to look at the trend across time to see whether the concentration of different nutrients in different lakes changes over time. 
