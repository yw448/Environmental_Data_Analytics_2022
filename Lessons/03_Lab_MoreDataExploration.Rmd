...
```{r}
high_temp=c(44,48,34,43,51,51,59,66,59,44)
low_temp=c(27,27,15,25,25,33,48,47,26,22)
```
...
high_c=f_to_c(high_temp)
low_c=f_to_c(low_temp)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
ten_day_forecast=data.frame("high.temp.F"=high_temp,"low.temp.F"=low_temp,"high.temp.c"=high_c,"low.temp.c"=low_c)
```
...
```{r}
summary(ten_day_forecast)
sd(ten_day_forecast$high.temp.F)
sd(ten_day_forecast$low.temp.F)
sd(ten_day_forecast$high.temp.c)
sd(ten_day_forecast$low.temp.c)
```
