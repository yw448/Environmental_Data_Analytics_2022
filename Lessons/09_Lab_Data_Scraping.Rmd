...
gage_data <- read.table(theURL,skip = 28,header=TRUE,sep='\t')  #/t=tab
...
```{r}
#change site number, period from 7 to 24
#Set the URL

theURL2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb' 

#Get the data, which starts on line 30
gage_data2 <- read.table(theURL2,skip = 28,header=TRUE,sep='\t')  #/t=tab

#Update the column headers
colnames(gage_data2) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
...
the_states <- the_website %>% html_nodes('.basic_table a') %>% html_text()
...
#3 Construct a dataframe from the values
  the_df=data.frame("states"=the_states,
                    "price"=as.numeric(the_price),
                    "capacity"=as.numeric(gsub(",","",the_capacity)),
                    "generation"=as.numeric(gsub(",","",net_generation)),
                    "total"=as.numeric(gsub(",","",total_retail)),
...
the_years=seq(2017,2020)

the_data=lapply(X=the_years,FUN=scrape.it) %>% bind_rows()
```
