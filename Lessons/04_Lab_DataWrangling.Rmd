...
NTL.phys.data.Tue <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: The rows are not ordered by years of each lake. 
...
NTL.phys.data.Tue.Jul.surfacetempsum <-
  NTL.phys.data.Tue.Jul %>%
  filter(depth == 0) %>%
...
NTL.phys.data.ex5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "measured")
