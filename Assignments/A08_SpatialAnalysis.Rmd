...
> ANSWER: The EPSG code is 4269. 
It is a angular coordinate units.
It is associated with NAD83
> 
...
>ANSWER: 
> new field: geometry
  disappeared field: dec_long_va & dec_lat_va
...
ggplot()+
  ...
  geom_sf(data=GageLocation.sf,color="purple",size=2)+
  ggtitle("NWIS Gage Locations in Nebraska",
          subtitle = "Yunting Wang")
...
Gage.merge=merge(x=GageLocation.sf,
      y=GageHeight,
      by.x="site_no",
      by.y="site_no")
...
mapview(counties_sf,col.regions="orange")+
  mapview(Gage.merge,cex="gage_ht",zcol="gage_ht")

#ggplot
ggplot()+
  geom_sf(data=counties_sf,fill="orange")+
  geom_sf(data=Gage.merge,aes(color=gage_ht,size=gage_ht))+
  scale_fill_gradient()
...
gage_utm <- st_transform(GageLocation.sf, c=32614)


#21 Plot the data
mapview(counties_sf_utm,col.regions="orange")+
  mapview(gage_utm,col.regions="darkblue")
...
Gage.intersect=gage_utm %>%
  st_intersection(Lancaster)

#24 Plot
mapview(counties_sf_utm,col.regions="grey")+
  mapview(Lancaster,col.regions="orange")+
  mapview(Gage.intersect,col.regions="darkgreen")
